...
// DO NOT CHANGE the order of this enum class
enum class Type {
...
public:
	 ...
	 * @return Symbol* variable symbol if exist, nullptr otherwise
	 */
	Symbol *get_local(const std::string &name) {
		level scope = scopes.front();
		auto it = scope.find(name);
		if (it != scope.end()) {
			return &it->second;
		}
